...
def parse_args():
    ...
    # model 
    parser.add_argument('--model_name', type=str, default='USVN',help='model')
    parser.add_argument('--encoder_name', type=str, default='densenet161',help='encoder_name')

    # select fold & seed num
    parser.add_argument('--seed_num', type=int, default=234,help='seed_num')
    ...
    parser.add_argument('--pooling_method', type=str, default='attn',help='pooling_method')
    parser.add_argument('--train_layer', type=str, default='all',help='train_layer')

    # save_path to save checkpoint
    parser.add_argument('--save_path', type=str, default='/data2/hoon2/Results/clip_base/multilabel_classification/',help='save_path')
    
    parser.add_argument('--base_path', type=str, default='/data2/hoon2/LUS_Dataset/csv_files/clip_multilabel_classification/',help='base_path')
    ...
    parser.add_argument('--total_epochs', type=int, default=100, help='the number of training epochs.')
...
def train():
    ...
    wandb.init(project='LUS_MICU_clip', config=config)
    wandb.run.name = f'seed_{args.seed_num}_test{args.model_test_rate}_{args.version}_{args.model_name}_{args.encoder_name}_{args.encoder_batch_size}_{args.task}_{args.model_output_class}_artifacts_{args.frame_size}_duplicate_{args.batch_size*args.accumulation_steps}batch_{args.pooling_method}_fold{args.fold_num}_{args.train_layer}_lr{args.lr}_train'
    wandb.run.save()
    ...
    # train csv_path
    train_csv_path = os.path.join(args.base_path, f'model_development_set/{args.version}/{args.model_output_class}_artifacts/test_{args.model_test_rate}/fold_{args.fold_num}/train.csv')

    # valid csv_path
    valid_csv_path = os.path.join(args.base_path, f'model_development_set/{args.version}/{args.model_output_class}_artifacts/test_{args.model_test_rate}/fold_{args.fold_num}/valid.csv')

    # make dataset
    train_dataset = video_dataset(train_csv_path, transforms = apply_transforms(mode='train'), img_size = args.frame_size, is_train = True)
    valid_dataset = video_dataset(valid_csv_path, transforms = apply_transforms(mode=None), img_size = args.frame_size, is_train = False)

    # class_0_weight, class_1_weight, class_2_weight, class_3_weight, sample_weights = calculate_class_weight(dataset = train_dataset)
    # sampler = WeightedRandomSampler(torch.DoubleTensor(sample_weights), len(train_dataset))
    # print(f"class_0_weights : {class_0_weight}, class_1_weight : {class_1_weight}, class_2_weight : {class_2_weight}, class_3_weight : {class_3_weight}")

    if args.model_name == 'USVN':
        # dataloader
    ...
    elif args.model_name == 'CNNLSTM' or args.model_name == 'C3D' or args.model_name == 'R2Plus1D_18' or args.model_name == 'R2Plus1D_34' or args.model_name == 'mvit_v2_s' or args.model_name == 'mvit_v1_b' or args.model_name == 'swin3d_s' or args.model_name == 'swin3d_b':
...
#     print('Count of using GPUs:', torch.cuda.device_count())
    ...
    if args.model_name == 'USVN':
        ...
        if args.encoder_name == 'imgnet_init_densenet161':
            encoder = timm.create_model('densenet161', pretrained=True, num_classes=300)
        
        elif args.encoder_name == 'densenet161_scratch':
            encoder = timm.create_model('densenet161', pretrained=False, num_classes=300)
            
        elif args.encoder_name == 'imgnet_init_resnet50':
            encoder = timm.create_model('resnet50', pretrained=True, num_classes=300)
...
#         model = MedVidNet(args.train_layer, encoder, args.num_heads, pooling_method = args.pooling_method)
...
#         model = C3D(num_classes=args.model_output_class, pretrained=False)  # C3D 모델 초기화
#         model = torch.nn.DataParallel(model, device_ids=[0, 1]).to(device=device)

#     elif args.model_name == "R2Plus1D":
#         model = R2Plus1DClassifier(num_classes=args.model_output_class, layer_sizes=(2, 2, 2, 2))
#         model = torch.nn.DataParallel(model, device_ids=[0, 1]).to(device=device)
    ...
    # R(2+1)D-18
        ...
        model = model.to(device)
    # R(2+1)D-34
    elif args.model_name == 'R2Plus1D_34':
...
#     optimizer = optim.AdamP(model.parameters(), lr=LR, weight_decay = 1.e-3)
    ...
    for epoch in tqdm(range(0, args.total_epochs + 1)):
        ...
        for batch_ndx, data in enumerate(train_dataloader):
            ...
            train_imgs, train_labels, _ = data
...
#             print(train_labels.shape)
            ...
            # model output
            if args.model_name == 'CNNLSTM' or args.model_name == 'C3D' or args.model_name == 'R2Plus1D_18' or args.model_name == 'R2Plus1D_34':
                y_preds = model(train_imgs)
            if args.model_name == 'USVN':
                y_preds, attentions = model(train_imgs, num_frames)
            if args.model_name == 'mvit_v2_s' or args.model_name == 'mvit_v1_b' or args.model_name == "swin3d_s" or args.model_name == "swin3d_b":
                train_imgs = train_imgs.permute(0, 2, 1, 3, 4)
                y_preds = model(train_imgs)
...
#             train_loss.backward()
            ...
            train_loss.backward()
            # Gradient accumulation: optimizer.step() and zero_grad() every `accumulation_steps` batches
        ...
        with torch.no_grad():
            ...
            epoch_accs_val.append(epoch_acc_valid)
...
#                 best_thresholds = current_thresholds
                ...
                checkpoint(args.seed_num, args.batch_size*args.accumulation_steps, args.version, 
                           args.frame_size, args.train_layer, args.model_test_rate, 
                           checkpoint_std_loss, model, args.model_output_class, args.save_path, 
                           args.model_name, args.encoder_name, args.pooling_method, 
                           args.fold_num, best_epoch, best_valid_loss, 
                           best_valid_acc, best_valid_auc, best_thresholds, args.lr)
            '''
