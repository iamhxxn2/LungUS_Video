...
def parse_args():
    ...
    parser.add_argument('--save_path', type=str, default='/home/work/LUS/Results/clip_base/multilabel_classification/',help='save_path')
...
def train():
    ...
    wandb.run.name = f'seed_{args.seed_num}_test{args.model_test_rate}_{args.data_type}_{args.version}_{args.model_name}_{args.encoder_name}_{args.encoder_batch_size}_{args.task}_{args.model_output_class}_artifacts_{args.frame_size}_duplicate_{args.batch_size*args.accumulation_steps}batch_{args.pooling_method}_fold{args.fold_num}_{args.train_layer}_lr{args.lr}_train'
    wandb.run.save()
    ...
    # train csv_path
    train_csv_path = os.path.join(args.base_path, f'{args.data_type}/{args.version}/{args.model_output_class}_artifacts/test_{args.model_test_rate}/fold_{args.fold_num}/train.csv')

    # valid csv_path
    valid_csv_path = os.path.join(args.base_path, f'{args.data_type}/{args.version}/{args.model_output_class}_artifacts/test_{args.model_test_rate}/fold_{args.fold_num}/valid.csv')
    ...
    if args.model_name == 'USVN' or args.model_name == 'BiLSTM_USVN':
        # dataloader
    ...
    elif args.model_name == 'Transformer_USVN' or args.model_name == 'CNNLSTM' or args.model_name == 'CNNTransformer' or args.model_name == 'C3D' or args.model_name == 'R2Plus1D':
...
#     print('Count of using GPUs:', torch.cuda.device_count())
    ...
    # image encoder
    if args.model_name == 'USVN' or args.model_name == 'Transformer_USVN' or args.model_name == 'BiLSTM_USVN':
    ...
    #         encoder = torch.hub.load('pytorch/vision:v0.10.0', 'densenet121', pretrained=False)
        ...
        if args.encoder_name == 'densenet161':
            ...
            check_point = torch.load(weight_path, map_location='cpu', strict = False)
...
#     elif args.model_name == "Transformer_USVN":
#         num_frames = [30]*(args.batch_size // num_gpu)

#         model = Transformer_USVN(encoder=encoder, num_heads=args.num_heads)
#         model = torch.nn.DataParallel(model, device_ids=[0, 1]).to(device=device)
...
#         model = torch.nn.DataParallel(model, device_ids=[0, 1]).to(device=device)
    ...
    if args.model_name == "USVN":
        ...
        model = USVN(args.train_layer, encoder, args.num_heads, pooling_method = args.pooling_method)
...
#             print(train_labels.shape)
            ...
            # model output
            if args.model_name == 'CNNLSTM' or args.model_name == 'C3D' or args.model_name == 'R2Plus1D':
...
#                 best_thresholds = current_thresholds
                ...
                checkpoint(args.batch_size*args.accumulation_steps, args.version, args.frame_size, args.data_type, args.train_layer, args.model_test_rate, checkpoint_std_loss, model, args.model_output_class, args.save_path, args.model_name, args.encoder_name, args.encoder_batch_size, args.pooling_method, args.fold_num, best_epoch, best_valid_loss, best_valid_acc, best_valid_auc, best_thresholds, args.lr)
            '''
