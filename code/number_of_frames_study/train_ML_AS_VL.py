...
def train():
    ...
    wandb.run.name = f'seed_{args.seed_num}_test{args.model_test_rate}_{args.data_type}_{args.version}_clip{args.frame_num}_{args.model_name}_{args.encoder_name}_{args.encoder_batch_size}_{args.task}_{args.model_output_class}_artifacts_{args.frame_size}_duplicate_{args.batch_size*args.accumulation_steps}batch_{args.pooling_method}_fold{args.fold_num}_{args.train_layer}_lr{args.lr}_train'
    wandb.run.save()
...
#         model = torch.nn.DataParallel(model, device_ids=[0, 1]).to(device=device)
    ...
    elif args.model_name == "BiLSTM_USVN":
        num_frames = [args.frame_num]*args.batch_size
        
        model = BiLSTM_USVN(train_layer='all', encoder=encoder, num_heads=args.num_heads)
        model = model.to(device)
...
#             print(train_labels.shape)
            ...
            # model output
            if args.model_name == 'CNNLSTM' or args.model_name == 'CNNTransformer' or args.model_name == 'C3D' or args.model_name == 'R2Plus1D':
...
#                 best_thresholds = current_thresholds
                ...
                checkpoint(args.batch_size*args.accumulation_steps, args.version, args.frame_num, args.frame_size, args.data_type, args.train_layer, args.model_test_rate, checkpoint_std_loss, model, args.model_output_class, args.save_path, args.model_name, args.encoder_name, args.encoder_batch_size, args.pooling_method, args.fold_num, best_epoch, best_valid_loss, best_valid_acc, best_valid_auc, best_thresholds, args.lr)
            '''
